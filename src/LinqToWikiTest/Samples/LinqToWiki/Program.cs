...
namespace LinqToWiki.Samples
{
    static class Program
    {
        private static void Main()
        {
            ...
            var wiki = new Wiki("LinqToWiki.Samples", "zh.asoiaf.wikia.com", "/api.php");
            // Login(wiki, "username", "password");

            //var allPagesources =
            //    wiki.Query.allpages().Where(p => p.filterredir == allpagesfilterredir.nonredirects).Pages;
            //var pages = allPagesources.Select(p => p.langlinks().Where(l = >l.))
            var pageSource = wiki.CreateTitlesSource("魔龙的狂舞-附录");
            var result =
                pageSource.Select(p => PageResult.Create(p.info, p.langlinks().Where(l => l.url).ToEnumerable()))
                    .ToList();
            //Compare(wiki);
